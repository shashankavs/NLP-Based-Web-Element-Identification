...
class PhraseNodeEvalRun(object):
    ...
    def eval(self, phrase, info):
        ...
        example = {"exampleId": "eval", "phrase": phrase, "version": "v6", "webpage": "eval.com", "xid": 1}
        example_path = join(os.environ['WEBREP_DATA'], 'phrase-node-dataset', 'data', 'eval.jsonl')
        info_path = join(os.environ['WEBREP_DATA'], 'phrase-node-dataset', 'infos', 'v6', 'info-eval.com.gz')
    ...
    def _process_one(self, examples):
        ...
        print(examples[0], "This is config")
        print(web_page, "This is a webpage")
        if not web_page:
            return stats

        logits, losses, predictions = self.model(web_page, examples)
        # print(examples, examples[0].get_web_page())
        # loss
